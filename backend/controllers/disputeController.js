...
exports.createDispute = async (req, res) => {
  ...
  await Notification.create({ userId: e.creatorId, type: 'DISPUTE_OPENED', payload: { disputeId: dispute._id } });
  await Notification.create({ userId: e.brandId, type: 'DISPUTE_OPENED', payload: { disputeId: dispute._id } });
  res.json(dispute);
};
...
exports.resolveDispute = async (req, res) => {
  ...
  try {
    ...
    await Notification.create({ userId: resolved.creatorId, type: 'DISPUTE_RESOLVED', payload: { disputeId: resolved._id, resolution: resolved.resolution } });
    await Notification.create({ userId: resolved.brandId, type: 'DISPUTE_RESOLVED', payload: { disputeId: resolved._id, resolution: resolved.resolution } });
